...
def enforce_title_style(prs, size_pt=TITLE_PT, font=TITLE_FONT):
            ...
            for para in txBody.iter(f'{{{A}}}p'):
                for run in para.findall(f'{{{A}}}r'):
                    if run.find(f'{{{A}}}rPr') is None:
                        ...
                        latin.set('typeface', font)
                        run.insert(0, rPr)
            # grow the box for the new wrap count (complete xfrm — never partial)
            if sh.width and sh.left is not None and sh.top is not None:
                ...
                if Emu(sh.height).inches < need:
                    sh.left, sh.top = sh.left, sh.top
                    sh.height = Inches(need)
            align_blade_to_title(slide)
            n_done += 1
            break
    return n_done
...
def reskin_deck(src_path, out_path, brand, extra_remap=None,
    ...
    strip_png = takeda._chrome_png(f'{brand}_BANNER')
    if strip_png:
        apply_footer_strip_locked(prs, strip_png)
    for slide in prs.slides:
    ...
    enforce_title_style(prs, title_pt, title_font)
