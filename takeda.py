...
# The DARKER step of the same ZS slate ramp (4F868E) — must land in the same brand
# family as 688A92 but one shade deeper (ramp rule 7: recolor ramps consistently).
_ZS_SLATE_TEAL_DARK = {
    'ENTYVIO':  '891515',   # deep crimson family step
    'FRUZAQLA': '0B0964',   # deep indigo
    'ICLUSIG':  '074F71',   # deep blue
    'ALUNBRIG': '005158',   # deep teal
}
for _b, _hex in _ZS_SLATE_TEAL_DARK.items():
    if _b in BRANDS:
        BRANDS[_b].setdefault('BRAND_REMAP', {}).setdefault('4F868E', _hex)
del _b, _hex

# Divider-slide background per brand (Mode B / Path A: reskin_deck stamps this as a
# slide-level solid bg on divider slides; the contrast pass then whitens titles).
# FRUZAQLA sampled from the brand reference cover (deep indigo core of the
# violet/blue radial identity) — the generic remap otherwise sends the source navy
# into the orchid family, which reads as off-brand/ENTYVIO-ish. ENTYVIO per the
# Fact Book near-black divider spec.
_BRAND_DIVIDER_BG = {
    'FRUZAQLA': '0B0964',   # deep indigo (violet/blue primary identity)
    'ENTYVIO':  '1A1628',   # near-black
}
for _b, _hex in _BRAND_DIVIDER_BG.items():
    if _b in BRANDS:
        BRANDS[_b].setdefault('BRAND_DIVIDER_BG', _hex)
del _b, _hex

# ── FRUZAQLA corrections (2026-07-29, ZS COGNITIVE conversion feedback) ──────────
# ZS's theme ABUSES its slots: accent1 = the ZS dark navy (used for full-height side
# panels), lt2 = ZS orange EC7200 (used as a text/fill ACCENT, not a background).
# The harvested remap sent accent1 → orchid (panels went magenta / "ENTYVIO-ish")
# and never touched lt2 (orange survived; the contrast pass then blackened orange
# headings). Rule (all brands): source accent colors land on brand PRIMARY colors,
# secondary only if contrast fails — including theme-slot-borne accents.
if 'FRUZAQLA' in BRANDS:
    BRANDS['FRUZAQLA'].setdefault('FRUQ_THEME_REMAP', {}).update({
        'accent1': '1A1628',   # ZS near-black TEXT slot — MUST STAY DARK (panels are
        #                          fixed shape-level via BRAND_DIVIDER_BG, never via slot)
        'accent2': '690588',   # ZS orange accent slot → PRIMARY violet (rule: primary preferred)
        'lt2':     '690588',   # ZS orange-as-accent slot → primary violet
    })
    # Text-accent preference for the contrast pass: when a text color must be
    # corrected on a light background, land on these (in order) before neutral dark.
    BRANDS['FRUZAQLA'].setdefault('BRAND_TEXT_ACCENTS', ['690588', '074F71', '743272'])
    # Title safe zone: the FRUQ banner's top-right corner wave reaches into the
    # title row; cap the title's right edge so text never collides with it.
    BRANDS['FRUZAQLA'].setdefault('BRAND_TITLE_SAFE', {'max_right_in': 10.4})
    # SVG icon recolor (icons are SVG picture parts — shape remaps can't reach
    # them; reskin_deck rewrites the SVG XML): ZS teal line icons → primary violet.
    BRANDS['FRUZAQLA'].setdefault('BRAND_ICON_RECOLOR', {'32A29B': '690588',
                                                         '3CA2A6': '690588'})

# ── CORPORATE (Takeda) Mode B entry (2026-07-29) — the registry only carried the
# sub-brands, so reskin_deck could not run corporate reformats. Palette per
# TAKEDA_PALETTE: red E1242A, deep red 891515, charcoal 34373F, gray family.
BRANDS.setdefault('CORPORATE', {})
BRANDS['CORPORATE'].setdefault('BRAND_REMAP', {
    'EC7200': 'E1242A', 'F7941D': 'E1242A', 'FF6600': 'E1242A',   # ZS oranges → Takeda red
    'EE1100': 'E1242A', 'FF0000': 'E1242A', 'C00000': '891515',
    '688A92': '891515', '4F868E': '891515',                        # ZS slate ramp → deep red
    '32A29B': 'E1242A', '3CA2A6': 'E1242A',                        # ZS teal accents → red
    '1A1628': '34373F',                                            # ZS near-black → corporate charcoal
})
BRANDS['CORPORATE'].setdefault('CORPORATE_THEME_REMAP', {
    'accent1': '34373F',   # ZS text slot → corporate charcoal (STAYS DARK — text rule)
    'accent2': 'E1242A',   # ZS orange accent slot → Takeda red
    'lt2':     'E1242A',
    'accent3': '891515',
})
BRANDS['CORPORATE'].setdefault('BRAND_DIVIDER_BG', '34373F')
BRANDS['CORPORATE'].setdefault('BRAND_TEXT_ACCENTS', ['E1242A', '891515', '34373F'])
BRANDS['CORPORATE'].setdefault('BRAND_DARK_TEXT', '34373F')   # contrast-pass neutral dark
BRANDS['CORPORATE'].setdefault('BRAND_ICON_RECOLOR', {'32A29B': '34373F',   # icons grey,
                                                      '3CA2A6': '34373F'})  # matching dividers
# White corporate cover: no art asset — just the Takeda logo top-left (repo file)
BRANDS['CORPORATE'].setdefault('BRAND_COVER_LOGOS', [('takeda_logo.png', 0.55, 0.35, 1.5)])

# ── ICLUSIG corrections (2026-07-29, ZS COGNITIVE conversion feedback) ───────────
# ICLUSIG had NO *_THEME_REMAP at all, so ZS's abused theme slots (accent1 = navy
# side panels, accent2/lt2 = orange accents) passed through the retheme untouched
# and the contrast pass then neutralized them. Identity per official reference
# cover: sage 95B9BF + yellow chevron FCD634 on white; primary blues for text.
if 'ICLUSIG' in BRANDS:
    BRANDS['ICLUSIG'].setdefault('ICLUSIG_THEME_REMAP', {
        'accent1': '1A1628',   # ZS near-black TEXT slot — MUST STAY DARK (see rule:
        #                          black in reference stays black; panels are fixed
        #                          shape-level via BRAND_DIVIDER_BG, never via this slot)
        'accent2': '1077BE',   # ZS orange accent slot → primary blue (yellow fails on white)
        'lt2':     '1077BE',   # ZS orange-as-accent slot → primary blue
        'accent3': '074F71',   # ZS teal slot → deep blue
    })
    BRANDS['ICLUSIG'].setdefault('BRAND_DIVIDER_BG', '1077BE')
    BRANDS['ICLUSIG'].setdefault('BRAND_TEXT_ACCENTS', ['1077BE', '074F71', '01AFD7'])
    BRANDS['ICLUSIG'].setdefault('BRAND_ICON_RECOLOR', {'32A29B': 'F6BB00',
                                                        '3CA2A6': 'F6BB00'})
    # Cover: real slide-master export in the repo (exact case matters on GitHub raw)
    BRANDS['ICLUSIG'].setdefault('BRAND_COVER_ASSET', 'Iclusig_cover.png')
    # The ICLUSIG cover art is LIGHT (sage + yellow) — title sits in the sage zone
    # (white per brand reference); body/disclaimer sit over the pale chevron → dark.
    # Cover logos: (asset, x_in, y_in, w_in) — Takeda logo bottom-left + brand logo
    # bottom-right, inside the artwork's white bottom strip (mirrors the reference).
    BRANDS['ICLUSIG'].setdefault('BRAND_COVER_LOGOS', [
        ('takeda_logo.png',  0.55, 6.72, 1.5),
        ('ICLUSIG_LOGO',     10.9, 6.72, 1.9),
    ])
    # Visual brand references (repo images; consulted via brand_visual_refs()):
    BRANDS['ICLUSIG'].setdefault('BRAND_VISUAL_REFS',
        ['Iclusig_palette.png', 'Iclusig_cover_reference.png'])
    BRANDS['ICLUSIG'].setdefault('BRAND_COVER_TEXT', {'title': 'FFFFFF',
                                                      'body':  '074F71'})
    # Branded cover: full-bleed radial identity art stamped on the cover slide in
    # Mode B (reskin_deck). Asset resolves LOCAL-FIRST then from the GitHub repo via
    # takeda_remote.get() — drop the real brand artwork in the repo under this name
    # to override the procedural default.
    BRANDS['FRUZAQLA'].setdefault('BRAND_COVER_ASSET', 'fruzaqla_cover.png')
    BRANDS['FRUZAQLA'].setdefault('BRAND_VISUAL_REFS',
        ['fruzaqla_palette.png', 'fruzaqla_cover_reference.png'])
...
# Verified FRUZAQLA primaries (sampled from the official reference cover, 2026-07-29)
# belong in the QA palette — otherwise the gate flags the brand's own colors.
if 'FRUZAQLA' in BRAND_FULL_PALETTE:
    BRAND_FULL_PALETTE['FRUZAQLA'] |= {'690588', '0B0964', '040448', '331084'}
...
def brand_visual_refs(name):
    """Download this brand's visual reference images (palette screenshot, official
    cover) from the repo and return local paths — ready for the `view` tool.
    REQUIRED step of the Mode B visual brand check (see SKILL.md): render the
    converted deck, view these references, and compare cover art, palette usage,
    and accent hierarchy against them. Missing files are skipped, not fatal."""
    import takeda_remote
    paths = []
    for fn in get_brand(name).get('BRAND_VISUAL_REFS', []):
        try:
            paths.append(takeda_remote.get(fn))
        except Exception:
            pass
    return paths
